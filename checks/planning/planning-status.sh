...
# std: yoke-sdk-python:planning-status.01
...
# std: yoke-sdk-python:planning-status.02
check_granted_nothing_and_silent_without_a_credential() {
  ...
  local out status
  out="$(GH_TOKEN="" GITHUB_REPOSITORY=yoke-project/yoke-sdk-python "$planning_script" push 2>&1)"
  status=$?
...
# std: yoke-sdk-python:planning-status.03
...
# std: yoke-sdk-python:planning-status.04
...
JSON
  named="$(items_named_by_push "$event" "yoke-project/yoke-sdk-python")"
  rm -rf "$tmp"

  local want
  want="$(printf '%s
' "yoke-project/meta-yoke#7" "yoke-project/yoke-sdk-python#3" | sort)"
  [[ "$(sort <<<"$named")" == "$want" ]] || { echo "it read: $(tr '\n' ' ' <<<"$named")"; return 1; }
}

# std: yoke-sdk-python:planning-status.05
...
SH
  ...
  out="$(PATH="$tmp:$PATH" GH_TOKEN=a-credential PLANNING_ORG=yoke-project PLANNING_PROJECT=1 \
    GITHUB_REPOSITORY=yoke-project/yoke-sdk-python GITHUB_EVENT_PATH="$tmp/event.json" \
