...
| **Planning item** | yoke-project/yoke-sdk-python#3 |

## yoke-sdk-python:planning-status.01 — the workflow runs on the work, and runs nothing of its own
...
## yoke-sdk-python:planning-status.02 — it is granted nothing, and with no credential it does nothing
...
## yoke-sdk-python:planning-status.03 — an item moves forward, and never back
...
## yoke-sdk-python:planning-status.04 — the items are read from the change, and never invented
...
## yoke-sdk-python:planning-status.05 — a query that fails moves nothing, and says so
