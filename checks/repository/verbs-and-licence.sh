...
# std: yoke-sdk-python:verbs-and-licence.01
check_verbs_defined() {
...
# std: yoke-sdk-python:verbs-and-licence.02
...
# std: yoke-sdk-python:verbs-and-licence.03
check_container_build() {
  ...
  "$engine" build -q -t yoke-sdk-python-check-build -f "$root/Containerfile" "$root" >/dev/null \
    || { echo "the image did not build"; return 1; }
  "$engine" run --rm yoke-sdk-python-check-build just build >/dev/null \
    || { echo "build failed inside the container"; return 1; }
}

# std: yoke-sdk-python:verbs-and-licence.04
check_licence() {
...
# std: yoke-sdk-python:verbs-and-licence.05
check_develop_floor() {
  [[ -f "$root/justfile" ]] || { echo "no justfile"; return 1; }
  local out
...
# std: yoke-sdk-python:verbs-and-licence.06
check_fmt_fails_on_unformatted() {
  [[ -f "$root/justfile" ]] || { echo "no justfile"; return 1; }
  local tmp out
  tmp="$(mktemp -d)"
  cp -a "$root/." "$tmp/"
  printf 'x = [1,2 ,3]\n' > "$tmp/unformatted_check.py"
  if out="$(cd "$tmp" && just fmt 2>&1)"; then
    rm -rf "$tmp"; echo "fmt passed on an unformatted file"; return 1
  fi
  rm -rf "$tmp"
  [[ "$out" == *unformatted_check.py* ]] || { echo "fmt failed without naming the file"; return 1; }
}
